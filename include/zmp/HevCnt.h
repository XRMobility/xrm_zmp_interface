#ifndef __HEV_CNT_H__
#define __HEV_CNT_H__
...
struct OtherInf
{
    ...
    zmp::minivan::LIGHT_STATE light; // 0x57F ライト状態 灯光状态。
    int level;                       // 0x5A4 燃料の残量 剩余燃料量。
    zmp::minivan::DOOR_STATE door;   // 0x5B6 ドアの状態 车门状态。
...
class HevCnt : public zmp::minivan::ChangeStateObserver
{
public:
    HevCnt();
    virtual ~HevCnt();
    ...
    void SndDiagReq(zmp::minivan::HEV_ECU kind);
    void SndDiagClear(zmp::minivan::HEV_ECU kind);
    void SndErrReq();
    void SndErrClear();
    void SndVersionReq();

    void GetConfig(zmp::minivan::HEV_CONFIG kind);
    void SetConfig(zmp::minivan::HEV_CONFIG kind, int val);
    void SetConfig(zmp::minivan::HEV_CONFIG kind, float val);
    void SaveConfig();

private:
    zmp::minivan::HevControl *_hevCnt;
    zmp::minivan::CANUSBZ *_canCom;
